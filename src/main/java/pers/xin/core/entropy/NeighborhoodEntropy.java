 ...
 */
public class NeighborhoodEntropy implements Entropy{
    ...
    public NeighborhoodEntropy(double delta, Instances data) {
        this.delta = delta;
        this.m_data = data;
    ...
    public NeighborhoodEntropy(double delta, Instances data, Instances normalize_data) {
        this(delta,data);
    ...
    public double entropy(Set<Integer> indices){
        ...
        Neighborhood neighborhoodHelper = new Neighborhood2NORM(normalize_data,delta,indices);
        for (int i = 0; i < num; i++) {
            int numNeighbor = 0;
            for (int j = 0; j < num; j++) {
//                if(i==j) continue;
    ...
    public double conditionalEntropy(Set<Integer> indicesR, Set<Integer> indicesS){
        ...
        Neighborhood neighborhoodHelperRJointS = new Neighborhood2NORM(normalize_data,delta,
                indicesRUnionS);
        Neighborhood neighborhoodHelperS = new Neighborhood2NORM(normalize_data,delta,indicesS);
        for (int i = 0; i < num; i++) {
...
//                if(i==j) continue;
    ...
    public double mutualInformation(Set<Integer> indicesR, Set<Integer> indicesS){
        ...
        indicesRUnionS.addAll(indicesR);
        indicesRUnionS.addAll(indicesS);
        Neighborhood neighborhoodHelperRJointS = new Neighborhood2NORM(normalize_data,delta,indicesRUnionS);

        Neighborhood neighborhoodHelperR = new Neighborhood2NORM(normalize_data,delta,indicesR);
